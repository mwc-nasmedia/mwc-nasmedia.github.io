...
$(document).off("keyup", '.number').on('keyup', '.number', function() {
	...
	if ($(this).attr('maxsize') != undefined && Number(sVal) > Number($(this).attr('maxsize'))) {
		sVal = $(this).attr('maxsize');
	}
	...
	if(tooltip){
		$(tooltip).each(function(idx, val) {
			...
			if(typeof title !=='undefined'){
				$(val).tooltip({
					title: title,
					//customClass: "testSsong",
					popperConfig : function(defaultBsPopperConfig) {
						const sParentTd = $(val).parent();
						if (sParentTd.hasClass("ellipsis-300") === true) {
							const sTooltipId = $(val).attr("aria-describedby");
							$("#" + sTooltipId).append('<span id="tooltip_width_' + sTooltipId + '" style="visibility:hidden; position:absolute; top:-9999px; font-size:.75rem;"></span>');
							$("#tooltip_width_" + sTooltipId).text($(val).text());

							/*const nX	= setFloor($(val).prop('offsetWidth') - $(val).offset().left);
							 const nY	= setFloor($(window).height()-$(val).offset().top+ $(val).outerHeight()) * -1;
							 $("#" + sTooltipId).attr('style', "transform: translate3d(" + nX + "px," + nY + "px, 0) !important");*/
							const sTooltipParentTdWidth = sParentTd.outerWidth();
							const sTooltipWidth         = $("#tooltip_width_" + sTooltipId).outerWidth();


							if (sTooltipWidth >= 300) {
								//$("#" + sTooltipId).addClass("testSsong");
								$("#" + sTooltipId).prop('style', "left: -200px !important;");
							}

							//alert($("#" + sTooltipId).offset().left);
						}
					}
				});
			}
