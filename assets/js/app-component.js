 ...
 * options.nInputWidth			= (number)input 박스 넓이 (미설정 시 sDateType에 따라 디폴트 넓이 지정)
 */
...
$.fn.setDatePicker = function(options) {
	...
	//placeholder 체크
	if (typeof container.attr("placeholder") == "undefined") {
		container.attr("placeholder", "날짜 선택");
	}
	...
	if (options.bInitialize) {
		pickerOption.startDate = options.sStartDate;
		pickerOption.endDate = options.sEndDate;
	}

	container.daterangepicker(pickerOption, function(start, end) {
		...
		if (this.singleDatePicker) {
			container.val(selectedStartDate);
		} else {
			container.val(selectedStartDate + ' ~ ' + selectedEndDate);
		}
...
const setDatepickerRange = function (container, term, type, sMinDate, sMaxDate) {
	var $checkinInput  = $('input.start_datepicker', container)
	var $checkoutInput = $('input.end_datepicker', container)
	var checkInPicker  = $checkinInput.parent().data('daterangepicker')
	var fm               = 'YYYY-MM-DD'
	if (checkInPicker.timePicker) {
		fm = 'YYYY-MM-DD HH:mm';
	}
	if (checkInPicker) {
		if (type === 'prev') {
			checkInPicker.startDate.subtract(1, term).startOf(term);
			checkInPicker.setEndDate(moment(checkInPicker.startDate).endOf(term));
		} else if (type === 'next') {
			checkInPicker.startDate.add(1, term).startOf(term);
			checkInPicker.setEndDate(moment(checkInPicker.startDate).endOf(term));
		} else if (type === 'range') {
			checkInPicker.setStartDate(moment(sMinDate));
			checkInPicker.setEndDate(moment(sMaxDate));
		} else if (type === 'prev_term') {
			checkInPicker.setStartDate(moment().subtract(1, term).startOf(term));
			checkInPicker.setEndDate(moment().subtract(1, term).endOf(term));
		} else {
			checkInPicker.setStartDate(moment().startOf(term));
			checkInPicker.setEndDate(moment().endOf(term));
		}
		var selectedStartDate = checkInPicker.startDate.format(fm)
		var selectedEndDate   = checkInPicker.endDate.format(fm)

		$checkinInput.val(selectedStartDate);
		$checkoutInput.val(selectedEndDate);

		var checkOutPicker = $checkoutInput.parent().data('daterangepicker')
		checkOutPicker.setStartDate(checkInPicker.startDate);
		checkOutPicker.setEndDate(checkInPicker.endDate);
	}
...
const setDatePickerOptions = function(options) {
	...
	options.sClass				= (typeof options.sClass === "string") ? options.sClass : "";
	...
	if (options.bSingleDatepicker) {
		options.bInitialize = options.bAutoUpdateInput = (typeof options.sStartDate === "string");
	} else {
		options.bInitialize = options.bAutoUpdateInput = (typeof options.sStartDate === "string" && typeof options.sEndDate === "string");
	}

	options.bResetBtn = !options.bInitialize;
	...
	options.locale = {
		...
		"separator"        : " ~ ",
		"applyLabel"       : '적용',
		"cancelLabel"      : options.bResetBtn ? '초기화' : '취소',
		"fromLabel"        : "From",
		"toLabel"          : "To",
	...
	let nWidth = options.sDateType == "day" ? 230 : 300;
	if (options.bSingleDatepicker) {
		nWidth /= 2;
	}

	options.nInputWidth = (typeof options.nInputWidth === "number") ? options.nInputWidth : nWidth;
...
const setDatePickerTpl = function(oContainer, options) {
	oContainer.css("width", options.nInputWidth);
	let defTpl = '<input type="hidden"  name="' + options.sStartDateName + '" id="' + options.sStartDateId + '" class="start_datepicker ' + options.sClass + '" value="' + options.sStartDate + '" >';
	if (!options.bSingleDatepicker) {
		defTpl += '<input type="hidden"  name="' + options.sEndDateName + '"  id="' + options.sEndDateId + '" class="end_datepicker hasDatepicker ' + options.sClass + '" value="' + options.sEndDate + '">';
	}
